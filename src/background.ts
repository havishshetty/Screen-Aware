...
async function updateTimeForDomain(domain: string, timeToAdd: number): Promise<void> {
    try {
        if (!domain || timeToAdd <= 0) return;

        const data = await chrome.storage.local.get(domain);
        const currentTime: number = data[domain] || 0;
        const newTime: number = currentTime + timeToAdd;
        
        // Reset if time is over 24 hours (prevents overflow)
        const maxDailyTime: number = 24 * 60 * 60 * 1000; // 24 hours in milliseconds
        const timeToStore: number = newTime > maxDailyTime ? timeToAdd : newTime;
...
async function checkTimeLimit(domain: string, timeSpent: number): Promise<void> {
    try {
        const data = await chrome.storage.local.get('timeLimits');
        const timeLimits: TimeLimits = data.timeLimits || {};
...
// Reset daily totals
async function resetDailyTotals(): Promise<void> {
    try {
        const data: StorageData = await chrome.storage.local.get(null);
        const preservedData: StorageData = {};
...
async function handleWindowFocus(windowId: number): Promise<void> {
    try {
...
setInterval(async () => {
    try {
        ...
        const deleteKeys = Object.keys(data).filter(key => 
